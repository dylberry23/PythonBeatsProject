        ...
        for j in range(instruments):
            if clicks[j][i] == -1:
                ...
                if actives[j] == 1:
                    ...
                    color = dark_gray
            rect = pygame.draw.rect(screen, color, [i * ((WIDTH - 200) // beats) + 200, (j * 100) + 5, 
                                                    ((WIDTH - 200) // beats) - 10, ((HEIGHT - 200)//instruments) - 10], 0, 3)
            rect = pygame.draw.rect(screen, gold, [i * ((WIDTH - 200) // beats) + 200, (j * 100), 
                                                    ((WIDTH - 200) // beats), ((HEIGHT - 200)//instruments)], 5, 5)
            rect = pygame.draw.rect(screen, black, [i * ((WIDTH - 200) // beats) + 200, (j * 100), 
...
while run:
    ...
    beats_add_rect = pygame.draw.rect(screen, gray, [785, HEIGHT - 150, 48, 48], 0, 5)
    beats_sub_rect = pygame.draw.rect(screen, gray, [785, HEIGHT - 100, 48, 48], 0, 5)
    add_text2 = medium_font.render('+1', True, white)
    sub_text2 = medium_font.render('-1', True, white)
    screen.blit(add_text2, (795, HEIGHT - 140))
    screen.blit(sub_text2, (800, HEIGHT- 90))
    #instrument rects
    instrument_rects = []
    for i in range(instruments):
